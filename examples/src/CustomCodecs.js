...
function data_decode(v) {
  ...
  let status_result = Stdlib_Option.getOr(Stdlib_Option.map(v["status"], decoderStatus), Spice.error(undefined, "status" + " missing", v));
  if (status_result.TAG === "Ok") {
    return {
      TAG: "Ok",
      _0: {
        status: status_result._0
      }
    };
  }
  let e = status_result._0;
