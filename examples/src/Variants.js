...
function user_decode(v) {
  ...
  if (id.TAG === "Ok") {
    ...
    if (nickname.TAG === "Ok") {
      let language = Stdlib_Option.getOr(Stdlib_Option.map(v["language"], language_decode), Spice.error(undefined, "language" + " missing", v));
      if (language.TAG === "Ok") {
        return {
          TAG: "Ok",
          _0: {
      ...
      let e = language._0;
      return Spice.error("." + ("language" + e.path), e.message, e.value);
    }
    let e$1 = nickname._0;
    return Spice.error("." + ("nickname" + e$1.path), e$1.message, e$1.value);
  }
  let e$2 = id._0;
