              ...
              [
                (
                  Asttypes.Nolabel,
                  {
                    ...
                    %expr
                    Belt.Array.getExn(jsonArr, [%e idx]);
                  },
                ),
              ],
            )
  ...
  | Rtag({txt, loc}, _, coreTypes) =>
    ...
    let decoded =
      ...
      | [] =>
        ...
        %expr
        Belt.Result.Ok([%e resultantExp]);
      | _ => generateArgDecoder(generatorSettings, args, txt)
      };

    {
      pc_lhs:
        Pconst_string(txt, Location.none, None)
        |> Pat.constant
        |> (v => Some(v) |> Pat.construct(lid("Js.Json.JSONString"))),
      pc_guard: None,
      pc_rhs:
        switch%expr (Js.Array.length(tagged) !== [%e argLen]) {
        | true =>
          Decco.error(
...
let generateUnboxedDecode = (generatorSettings, row) => {
  ...
  | Rtag({txt, loc}, _, args) =>
    ...
    | [a] =>
      ...
      switch (d) {
      | Some(d) =>
        let constructor = Exp.construct(lid(txt), Some([%expr v]));
...
let generateCodecs =
    ({doEncode, doDecode} as generatorSettings, rowFields, unboxed) => {
  let encoder =
    doEncode
  ...
  let decoderDefaultCase = {
    ...
    pc_rhs: [%expr
      Decco.error(
        "Invalid polyvariant constructor",
