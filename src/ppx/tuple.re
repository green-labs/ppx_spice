...
let generateEncoder = compositeEncoders => {
  ...
  %expr
  ([%p deconstructorPattern]) => [%e arrExp] |> Js.Json.array;
};
...
let generateDecodeSwitch = compositeDecoders => {
  ...
  compositeDecoders
  |> List.mapi(
       Decode_cases.generateErrorCase(List.length(compositeDecoders)),
     )
  |> List.append([
       generateDecodeSuccessCase(List.length(compositeDecoders)),
     ])
